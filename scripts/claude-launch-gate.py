...
MIN_SECONDS_BETWEEN_LAUNCHES = 60
...
def load_projects():
    ...
    for key, entry in projects.items():
        if not KEY_PATTERN.match(str(key)) or not isinstance(entry, dict):
            continue
...
def rate_limited(key):
    ...
    now = time.time()
    if isinstance(last, (int, float)) and now - last < MIN_SECONDS_BETWEEN_LAUNCHES:
        return True
    state[key] = now
    try:
...
def warp_config(key, project):
    ...
    body = "\n".join([
        ...
        "windows:",
        "  - tabs:",
        "      - title: %s" % json.dumps("Claude: %s" % key),
        "        layout:",
        "          cwd: %s" % json.dumps(project["path"]),
        "          commands:",
        "            - exec: %s" % json.dumps(command),
        "",
    ])
...
def main():
    ...
    key = words[1]
    if not KEY_PATTERN.match(key) or key not in projects:
        deny("unknown project")
    if rate_limited(key):
        deny("project %s was launched less than %ds ago" % (key, MIN_SECONDS_BETWEEN_LAUNCHES))
    ...
    if done.returncode != 0:
        log("launch-failed", "%s open exit %d" % (key, done.returncode))
        reply({"ok": False, "error": "could not open Warp"}, 1)
    log("launch", key)
    reply({"ok": True, "project": key, "note": "Warp window opened; attach from the Claude app"})
